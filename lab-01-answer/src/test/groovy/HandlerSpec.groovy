...
class HandlerSpec extends Specification {
  ...
  def "01 - can send a request to the root path"() {
    ...
    then: "a response is returned with body text of 'Hello Gr8Conf!'"
    testClient.response.body.text == "Hello Gr8Conf!" // `testClient.response` is the ReceivedResponse from the last request sent
  ...
  def "08 - can request a static asset"() {
    expect:
    getText("assets/js/app.js").trim() == "var message = 'Hello Gr8Conf!';"
  ...
  def "09 - can serve an index file"() {
    expect:
    getText("home/").trim() == "<html><body><p>Hello Gr8Conf!</p></body></html>"
